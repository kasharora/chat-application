...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      console.log('User not found:', email); // Debugging statement
      return res.status(400).json({ message: 'Invalid email or password' });
    }

    const isMatch = await bcrypt.compare(password, user.password);
    console.log('Password match status:', isMatch); // Debugging statement
    ...
    console.log('Token generated for:', email); // Debugging statement
